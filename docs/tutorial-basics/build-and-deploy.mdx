...
3. Builds a git-enabled Docker image once, then reuses it.
4. Runs `npm install` + `npm run build` inside that container.
5. Mirrors the built `build/` output into the web root with `rsync`.
   :::

## Prerequisites

- **Docker** installed and running on the host.
- **git** and **rsync** on the host (not in the container — see the notes below).
...
# Build the git-enabled image once; reuse it on later runs
if ! docker image inspect docusaurus-builder >/dev/null 2>&1; then
  echo "Image docusaurus-builder not found, building it..."
  docker build -t docusaurus-builder - <<'DOCKERFILE'
...
docker run --rm --dns 8.8.8.8 -v "$PWD":/app -w /app docusaurus-builder \
...
:::note[Heredoc marker]
Inside the script the Dockerfile heredoc uses `DOCKERFILE` as its marker rather
than `EOF`. If you paste this block into another heredoc (or a doc that already
uses `EOF`), distinct markers keep the two from colliding. On its own the
original `EOF` works fine.
:::
...
On the first run the script builds a small local image, `docusaurus-builder`,
that is just `node:lts-alpine` with git added. The `docker image inspect` guard
returns non-zero when the image is absent, which triggers the one-time build;
every run after that reuses the cached image and skips both the build and the
in-container `apk add`.

:::tip[Refreshing the image]
The guard checks whether the image _exists_, not whether it's _current_. Once
built it's cached indefinitely and won't pick up Node or git updates. When you
want a fresh Node version, force a rebuild:

```bash
docker build --no-cache -t docusaurus-builder - <<'EOF'
FROM node:lts-alpine
RUN apk add --no-cache git
EOF
```
...
:::warning[Image build and DNS]
`docker build` fetches git from Alpine's mirror, so it uses the same network
path that can fail with `DNS: transient error`. Unlike `docker run`, `docker
build` takes no `--dns` flag — if the build can't resolve the mirror, fix it at
the daemon level in `/etc/docker/daemon.json` rather than with a flag. This cost
is paid only once, when the image is first built.
:::
...
- **`npm install`, not `npm ci`.** `npm ci` deletes `node_modules` and
  ...
  upstream.
- **Cached build image.** After the first run there's no image build and no
  in-container git install.
...
on each `.mdx`. Stock `node:lts-alpine` has **no git binary** — which is exactly
why the script builds `docusaurus-builder` with git added. If you bypass that
image, either install git in the container or disable the feature in the docs
preset options:
...
`apk` couldn't download its package index — a DNS/egress problem, not a missing
package. For `docker run`, `--dns 8.8.8.8` addresses the common case; for
`docker build` (the image step), fix DNS at the daemon level. If the Alpine
mirror is blocked entirely, switch the base to `node:lts` (git built in) or
disable the last-update feature so git isn't needed at all.
:::
...
sed -i "s|__SITE_TITLE__|${SITE_TITLE}|g; s|__BOOTSTRAP_CSS__|${BOOTSTRAP_CSS}|g" "$OUTPUT"
...
location that matches `BASE_URL = /iiot-docs/` and
`DEST = /var/www/html/docs/iiot-docs`:
